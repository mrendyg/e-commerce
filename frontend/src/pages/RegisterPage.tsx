...
const RegisterPage = () => {
  ...
  const registerMutation = useMutation({
    mutationFn:() => registerRequest(email, name, last_name, username, password), 
    onSuccess: () => {
      navigate("/login");
    },
    onError: (error) => {
          ...
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
            ...
            {match() ? null : <p className="text-sm font-medium text-red-500">Passwords must match</p>}
              ...
              <p className="text-sm font-light text-gray-500 dark:text-gray-400">
                Have an account? <Link to={'/login'} className="font-medium text-primary-600 hover:underline dark:text-primary-500">Iniciar sesión</Link>
              </p>
            </form>
          </div>
        </div>
